...
# -------------------------------------------------- Codigo Principal --------------------------------------------------
# Función de tratamiento de imágenes
def Tratamiento(img):
    new_img = remove(img, bgcolor=(255,255,255,255)) 
    return new_img
...
def img_remover(files, set, dirname): # Funcion que quita el fondo de las imagenes y las guarda en formato .png
    ...
    for file in files: # Se ejecuta archivo por archivo
        ...
        img2 = Tratamiento(img) # Tratamiento de la foto 
...
def main():
    ...
    global root 
    root = tk.Tk() # Crea una ventana en Tkinter (No se puede adjuntar en Streamlit carpetas enteras, asi que uno de las opciones fue utilizar la librería tkinter)
